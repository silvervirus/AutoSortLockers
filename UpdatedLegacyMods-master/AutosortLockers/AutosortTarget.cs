...
namespace AutosortLockers
{
	public class AutosortTarget : MonoBehaviour
	{
...
#if BELOWZERO
                    var canvas = LockerPrefabShared.CreateCanvas(obj.transform).gameObject;
		...
		internal class AutosortStandingTargetBuildable
		{
			...
			{
				CoroutineTask<GameObject> task = AutosortStandingTargetBuildable.GetBuildPrefabAsync(TechType.SmallLocker);
				yield return task;
    ...
    public class AutosortTarget : MonoBehaviour
    {
        ...
        internal class AutosortTargetBuildable
        {
            ...
            public static void Patch()
            {
                ...
                if (AutosortConfig.EasyBuild.Value == true)
                {
                    customPrefab.SetRecipeFromJson("./BepinEx/plugins/AutosortLockers/Recipes/EZAutosortTarget.json");
                }
                else
                {
                    customPrefab.SetRecipeFromJson("./BepinEx/plugins/AutosortLockers/Recipes/AutosortTarget.json");
                };
        ...
        internal class AutosortStandingTargetBuildable
        {
            ...
            public static void Patch()
            {
                ...
                if (AutosortConfig.EasyBuild.Value == true)
                {
                    customPrefab.SetRecipeFromJson("./BepinEx/plugins/AutosortLockers/Recipes/EZAutosortTargetStanding.json");
                }
                else
                {
                    customPrefab.SetRecipeFromJson("./BepinEx/plugins/AutosortLockers/Recipes/AutosortTargetStanding.json");
                };
            ...
            public static IEnumerator SetTextMeshProPrefab()
            {
                CoroutineTask<GameObject> task = GetBuildPrefabAsync(TechType.SmallLocker);
                yield return task;
                GameObject prefab = task.GetResult();
                textPrefab = prefab.GetComponentInChildren<TextMeshProUGUI>();
            }
