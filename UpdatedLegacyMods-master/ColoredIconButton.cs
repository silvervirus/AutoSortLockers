...
namespace AutosortLockers
{
	public class ColoredIconButton : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
	{
		...
		{
			if (isEnabled && enabled && (eventData.button == PointerEventData.InputButton.Right) || (eventData.button == PointerEventData.InputButton.Left) || (GameInput.GetKeyDown(KeyCode.Joystick1Button1)))
            {
				onClick();
			}
		}
